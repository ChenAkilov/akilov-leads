...
async function getAll() {
  const { data: leads, error: e1 } = await supabase.from('leads').select('*').order('updated_at', { ascending: false }).limit(2000);
  if (e1) throw new Error(e1.message);
  const { data: working, error: e2 } = await supabase.from('working').select('*').order('updated_at', { ascending: false }).limit(5000);
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (op === 'add_or_update_lead') {
        const { lead } = body;
        if (!lead?.place_id) return res.status(400).json({ ok: false, error: 'missing lead/place_id' });
        const now = new Date().toISOString();
        const payload = {
          place_id: lead.place_id, name: lead.name || null, address: lead.address || null,
          ...
          categories: lead.categories || null, email: lead.email || null,
          enrich_emails: lead.enrich_emails || null, enrich_socials: lead.enrich_socials || null,
          updated_at: now
        };
        const { data, error } = await supabase.from('leads').upsert(payload, { onConflict: 'place_id' }).select().maybeSingle();
        if (error) return res.status(500).json({ ok: false, error: error.message });
        if (data?.id) await supabase.from('lead_actions').insert({ lead_id: data.id, action_type: 'upsert', payload });
      ...
      if (op === 'set_stage') {
        ...
        const { data, error } = await supabase.from('leads').update({ stage, updated_at: new Date().toISOString() }).eq('id', lead_id).select().maybeSingle();
        if (error) return res.status(500).json({ ok: false, error: error.message });
      ...
      if (op === 'add_note') {
        ...
        const { data, error } = await supabase.from('leads').update({ notes, updated_at: new Date().toISOString() }).eq('id', lead_id).select().maybeSingle();
        if (error) return res.status(500).json({ ok: false, error: error.message });
      ...
      if (op === 'set_working') {
        ...
        const payload = {
          place_id, is_working: !!value, name: name || null, address: address || null, country: country || null,
          lat: typeof lat === 'number' ? lat : null, lng: typeof lng === 'number' ? lng : null,
          updated_at: new Date().toISOString()
        };
        const { data, error } = await supabase.from('working').upsert(payload, { onConflict: 'place_id' }).select().maybeSingle();
        if (error) return res.status(500).json({ ok: false, error: error.message });
